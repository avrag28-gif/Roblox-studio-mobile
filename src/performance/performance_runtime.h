...
public:
 ...
 void SetDrawCalls(uint64_t n){frame_.drawCalls=n;}void SetTriangles(uint64_t n){frame_.triangles=n;}void SetMemory(size_t n){frame_.memoryBytes=n;}
 const FrameMetrics&Frame()const{return frame_;}bool Within(double budgetMs=16.67)const{return frame_.cpuMs<=budgetMs;}
};
}
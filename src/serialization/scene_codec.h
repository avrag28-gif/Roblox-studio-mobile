...
class SceneCodec {
 ...
 static void Write(const Instance&i,std::ostream&o,int depth){o<<depth<<'|'<<Hex(i.ClassName())<<'|'<<Hex(i.Name());if(auto*p=dynamic_cast<const BasePart*>(&i)){auto q=p->Position(),z=p->Size(),c=p->Color();o<<'|'<<q.x<<','<<q.y<<','<<q.z<<'|'<<z.x<<','<<z.y<<','<<z.z<<'|'<<c.r<<','<<c.g<<','<<c.b<<'|'<<p->Transparency()<<'|'<<p->Anchored()<<'|'<<p->CanCollide();}if(auto*s=dynamic_cast<const Script*>(&i))o<<'|'<<Hex(s->Source());o<<'\n';for(auto*x:i.GetChildren())Write(*x,o,depth+1);}
public:

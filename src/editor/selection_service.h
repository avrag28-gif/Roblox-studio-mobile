...
public:
 ...
 Instance* RayPick(const DataModel& game,const Ray& ray) {
   ...
   for(auto* i:game.GetDescendants()) if(auto* p=dynamic_cast<BasePart*>(i)) {
     Vector3 min=p->Position()-p->Size()*0.5f, max=p->Position()+p->Size()*0.5f;
     ...
     const float mn[3]={min.x,min.y,min.z}, mx[3]={max.x,max.y,max.z};
     bool ok=true; for(int a=0;a<3;a++){if(std::abs(d[a])<1e-6f){if(o[a]<mn[a]||o[a]>mx[a]){ok=false;break;}}else{float t1=(mn[a]-o[a])/d[a],t2=(mx[a]-o[a])/d[a];if(t1>t2)std::swap(t1,t2);tmin=std::max(tmin,t1);tmax=std::min(tmax,t2);if(tmin>tmax){ok=false;break;}}}

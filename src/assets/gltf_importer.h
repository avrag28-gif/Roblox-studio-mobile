...
class GltfImporter {
 ...
 static bool DecodeJson(const std::string&json,const std::string&body,GltfMesh&out,std::string&error){
  ...
  size_t accessor=0;if(!IntAfter(json,ap,accessor)){error="invalid POSITION accessor";return false;}
  std::string acc;if(!NthObject(json,"\"accessors\"",accessor,acc)){error="POSITION accessor missing";return false;}
  ...
  std::string view;if(!NthObject(json,"\"bufferViews\"",bv,view)){error="bufferView missing";return false;}
  size_t base=0,stride=0;GetField(view,"\"byteOffset\"",base);GetField(view,"\"byteStride\"",stride);if(stride==0)stride=12;
  if(body.empty()||base+off+count*stride>body.size()){error="POSITION data exceeds BIN chunk";return false;}out.positions.resize(count);
...
public:
 ...
 static bool ParseGlb(const std::string&bin,GltfMesh&out,std::string&error){
  ...
  if(json.empty()){error="GLB JSON chunk missing";return false;}while(!json.empty()&&(json.back()==' '||json.back()=='\t'||json.back()=='\n'||json.back()=='\0'))json.pop_back();return DecodeJson(json,body,out,error);
 }

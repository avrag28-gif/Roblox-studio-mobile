...
class LuauService{
 public:
  ...
  bool CompileAndRun(const std::string& source){
    ...
    lua_pushlightuserdata(L,game_);lua_setglobal(L,"__rsm_game"); lua_newtable(L); lua_pushcfunction(L,&GetServiceThunk); lua_setfield(L,-2,"GetService"); lua_setglobal(L,"game"); lua_newtable(L); lua_pushcfunction(L,&InstanceNewThunk); lua_setfield(L,-2,"new"); lua_setglobal(L,"Instance");

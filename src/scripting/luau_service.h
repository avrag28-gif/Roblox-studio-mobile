...
class LuauService{
 ...
 private:
  ...
  static int GetServiceThunk(lua_State*L){lua_getglobal(L,"__rsm_game"); auto*dm=static_cast<DataModel*>(lua_touserdata(L,-1)); lua_pop(L,1);if(!dm){lua_pushnil(L);return 1;}const char*n=luaL_checkstring(L,1);auto*i=dm->GetService(n);if(!i){lua_pushnil(L);return 1;}lua_pushlightuserdata(L,i);return 1;}

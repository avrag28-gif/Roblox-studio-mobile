...
class LuauService{
 ...
 private:
  ...
  static int GetServiceThunk(lua_State*L){lua_getglobal(L,"__rsm_game"); auto*dm=static_cast<DataModel*>(lua_touserdata(L,-1)); lua_pop(L,1);if(!dm){lua_pushnil(L);return 1;}int arg=lua_istable(L,1)?2:1; const char*n=luaL_checkstring(L,arg);auto*i=dm->GetService(n);if(!i){lua_pushnil(L);return 1;}lua_pushlightuserdata(L,i);return 1;}
  static int InstanceNewThunk(lua_State*L){const char*n=luaL_checkstring(L,1);auto p=InstanceFactory::New(n);if(!p){lua_pushnil(L);return 1;}auto*i=p.get();lua_getglobal(L,"__rsm_game"); auto*dm=static_cast<DataModel*>(lua_touserdata(L,-1)); lua_pop(L,1);if(dm){auto*ws=dm->GetService("Workspace");Instance::SetParent(std::move(p),ws?ws:dm);}lua_pushlightuserdata(L,i);return 1;}
#endif

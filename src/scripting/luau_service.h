...
class LuauService{
 public:
  ...
  bool CompileAndRun(const std::string& source){
    ...
    lua_newtable(L);lua_pushlightuserdata(L,game_);lua_pushcclosure(L,&GetServiceThunk,1);lua_setfield(L,-2,"GetService");lua_setglobal(L,"game");
    lua_newtable(L);lua_pushlightuserdata(L,game_);lua_pushcclosure(L,&InstanceNewThunk,1);lua_setfield(L,-2,"new");lua_setglobal(L,"Instance");

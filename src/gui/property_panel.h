...
#include <vector>
namespace rsm {
struct PropertyField { std::string name,type; bool writable=true; };
class PropertyPanel {
public:
 static std::vector<PropertyField> Describe(const Instance&i){
  ...
  if(auto*p=dynamic_cast<const BasePart*>(&i)){
   ...
   o.push_back({"CanCollide","bool",true});o.push_back({"CanTouch","bool",true});o.push_back({"CanQuery","bool",true});
   o.push_back({"Mass","float",true});o.push_back({"Shape","enum",true});
  }
  if(dynamic_cast<const Script*>(&i))o.push_back({"Source","string",true});
  return o;
 }
};
}
...
struct ProjectStore {
 ...
 static bool Write(const DataModel&dm,const std::string&path){auto tmp=path+".tmp";auto bak=path+".bak";std::ofstream f(tmp,std::ios::binary);if(!f)return false;auto s=Save(dm);f.write(s.data(),s.size());f.flush();f.close();std::error_code ec;if(std::filesystem::exists(path,ec))std::filesystem::copy_file(path,bak,std::filesystem::copy_options::overwrite_existing,ec);std::filesystem::rename(tmp,path,ec);if(ec){std::filesystem::remove(path,ec);std::filesystem::rename(tmp,path,ec);}return !ec;}
};
}
...
public final class AndroidAudioPlayer {
 ...
 public boolean load(InputStream in)throws IOException{
  ...
  int p=12,data=-1,dataLen=0; while(p+8<=b.length){String id=new String(b,p,4,"US-ASCII");int n=u32(b,p+4);if("fmt ".equals(id)&&n>=16){channels=u16(b,p+10);sampleRate=u32(b,p+12);bits=u16(b,p+22);}if("data".equals(id)){data=p+8;dataLen=Math.min(n,b.length-data);break;}p+=8+n+(n&1);}

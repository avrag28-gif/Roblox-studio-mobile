...
public final class AndroidAudioPlayer {
 ...
 public boolean load(InputStream in)throws IOException{
  ...
  int p=12,data=-1,dataLen=0; while(p+8<=b.length){String id=new String(b,p,4,"US-ASCII");int n=u32(b,p+4);if("fmt ".equals(id)&&n>=16){channels=u16(b,p+10);sampleRate=u32(b,p+12);}if("data".equals(id)){data=p+8;dataLen=Math.min(n,b.length-data);break;}p+=8+n+(n&1);}
  if(data<0||channels<1||channels>2||sampleRate<8000||sampleRate>192000)return false;pcm=new byte[dataLen];System.arraycopy(b,data,pcm,0,dataLen);return true;
 }
